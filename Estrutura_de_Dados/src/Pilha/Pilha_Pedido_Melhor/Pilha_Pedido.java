...
public class Pilha_Pedido {
	...
	// Criar função estaCheia() 
	// Criar função estaVazia() 
	...
	public void remover(String valorProcurado){
		...
		for(int i= 0; i<this.getTamanho(); i++) {
			...
			if(atual.getValor().equalsIgnoreCase(valorProcurado)) { //equalsignorecase é pra saber se o valor é igual ao valor procurado..mesmo que equivale
				if(atual==primeiro && atual == ultimo) {
					this.primeiro = null;
					this.ultimo = null;
				} else if(atual == primeiro) {
					this.primeiro = atual.getProximo();
					atual.setProximo(null);  // o atual agora vai ser o proximo nulo, assim apagando o ultimo
